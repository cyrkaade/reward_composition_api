...
def write_mujoco_component_summary(
    ...
    fieldnames = component_fieldnames(spec, custom_partial)
...
class MuJoCoComponentEvalCallback(BaseCallback):
    def __init__(
        self,
        log_path: Path,
        env_id: str,
    ...
    def _init_callback(self) -> None:
        ...
        if not self.log_path.exists():
            with self.log_path.open("w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=component_fieldnames(self.spec, self.custom_partial))
                writer.writeheader()
    ...
    def _on_step(self) -> bool:
        ...
        stats = evaluate_mujoco_components(
            self.model,
            self.env_id,
            self.spec,
