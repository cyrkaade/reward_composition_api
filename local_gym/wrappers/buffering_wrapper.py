...
class BufferingWrapper(VecEnvWrapper):
    ...
    def step_wait(self):
        ...
        for env_idx in range(self.num_envs):
            partial_reward = self.vec_cffns[env_idx].reward(v_obs[env_idx], self._saved_acts[env_idx], v_infos[env_idx]['game_over'], v_infos[env_idx]['awake']) if self.vec_cffns[env_idx] else 0
            expert_reward = self.vec_expfn[env_idx].reward(v_obs[env_idx], self._saved_acts[env_idx], v_infos[env_idx]['game_over'], v_infos[env_idx]['awake']) if self.vec_expfn else v_rews[env_idx]
            self.temp_trajectories[env_idx].push_state(v_obs[env_idx], self._saved_acts[env_idx], v_dones[env_idx], v_infos[env_idx], expert_reward, partial_reward)
            if v_dones[env_idx]:
