"""Read back the alignment x alpha grid on LunarLander.

Two figures from one pass over logs/align_*:

  *_curves.png   one panel per alignment rung, five alpha curves inside it,
...
Usage:
    python jobs/analyze_align.py [--logs logs] [--out logs/align]
"""
...
RUNGS = ["a00", "a30", "a50", "a75", "a95"]
RUNG_ALIGN = {"a00": 0.019, "a30": 0.308, "a50": 0.528, "a75": 0.748, "a95": 0.953}
...
def load_runs(root: Path):
    """run dirs are logs/align_<cell>_<arm>/align_<cell>_<arm>_seed<n>/"""
    runs = []
    for meta_path in sorted(root.glob("align_*/*/metadata.json")):
        cell_arm = meta_path.parent.parent.name[len("align_"):]
...
def main():
    ...
    ap.add_argument("--out", default="logs/align")
    ap.add_argument(
        "--complete-seeds-only",
        action="store_true",
    ...
    runs = load_runs(root)
    if not runs:
        raise SystemExit(f"no align runs found under {root}")
