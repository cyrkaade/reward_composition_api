...
#SBATCH --output=logs/slurm/align_%A_%a.out
#SBATCH --error=logs/slurm/align_%A_%a.err
...
# Alignment x alpha on LunarLander: 5 prior rungs x 5 weighted-sum alphas,
# q400, 1M timesteps, 5 seeds, plus true/vanilla references.
#
...
PARAMS_FILE="${PARAMS_FILE:-jobs/params_align.txt}"
LAUNCHER="${LAUNCHER-srun}"
EXPECTED_ROWS="${EXPECTED_ROWS:-135}"
TIMESTEPS="${TIMESTEPS:-1000000}"

if [ ! -f "$PARAMS_FILE" ]; then
  echo "$PARAMS_FILE missing; run: python jobs/make_params_align.py" >&2
  exit 2
fi
...
LOGDIR="logs/align_${CELL}_${ARM}"
RUNNAME="align_${CELL}_${ARM}_seed${SEED}"
